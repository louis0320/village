					...
					<div class="customer_ask2">
						...
						<div class="customer_input">
							<c:if test="${role=='admin' }"> 
                            	<input type="button" value="댓글" onclick='location.href="ReAddBoardCtrl.do?num=${list.seq}&ref=${list.ref}&re_step=${list.re_step }&re_level=${list.re_level}"'>
                            </c:if> 
							<input type="button" value="수정" onclick='location.href="UpdateBoardProCtrl.do?num=${list.seq }"'>
							<a href="DeleteBoardCtrl.do?num=${list.seq }" onclick="alert('삭제하시겠습니까?')"><input type="button" value="삭제"></a>
						</div>
					</div>
				</div>
				</c:forEach>
