...
public class BoardDAO {
	...
	public void reInsertBorad(BoardVO vo) {
		...
		try {
			...
			// 답글 데이터
			String sql = "insert into si_board values((select ifnull(max(seq),0)+1 from si_board b),?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP,0,?,?,?)";
			stmt = conn.prepareStatement(sql);
	...
	public int addboard(BoardVO vo) {
		...
		try {
			...
			String sql = "insert into si_board values(seq,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP,0,?,?,?)";
			stmt=conn.prepareStatement(sql);
			stmt.setString(1, vo.getType());
			stmt.setString(2, vo.getChoice());
			stmt.setString(3, vo.getTitle());
